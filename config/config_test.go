...
func TestNormalizeDockerSQLiteDSNUsesMountedDataDir(t *testing.T) {
	...
	Cfg = Config{StorageDriver: "sqlite", DatabaseDSN: "data/aicanvas.db?_pragma=busy_timeout(5000)"}

	normalizeDockerSQLiteDSN(appDataDir)

	want := filepath.Join(root, "data", "aicanvas.db") + "?_pragma=busy_timeout(5000)"
...
func TestNormalizeDockerSQLiteDSNLeavesLocalPathWithoutMountedDataDir(t *testing.T) {
	Cfg = Config{StorageDriver: "sqlite", DatabaseDSN: "data/aicanvas.db"}

	normalizeDockerSQLiteDSN(filepath.Join(t.TempDir(), "missing-data"))

	if Cfg.DatabaseDSN != "data/aicanvas.db" {
