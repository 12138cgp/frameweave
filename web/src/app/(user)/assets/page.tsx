...
import { Copy, Download, PencilLine, Search, Trash2, Upload } from "@/components/icons";
...
import { PromptFavoritesTab } from "@/app/(user)/canvas/components/canvas-prompt-favorites-tab";
...
const viewOptions = [
    { label: "我的素材", value: "assets" },
    { label: "收藏提示词", value: "favorites" },
];
...
function AssetsPageInner() {
    ...
    // 「收藏提示词」是另一个数据源（服务端 prompt_favorites 表，不是本地 assetStore），
    // 所以用视图切换而不是混进下面的 kindFilter —— 混在一起会让分页/搜索两套口径缠在一块。
    const [view, setView] = useState<"assets" | "favorites">("assets");
    ...
    const downloadImage = (asset: Asset) => {
        ...
        saveAs(asset.kind === "image" ? asset.data.dataUrl : asset.data.url, `${asset.title || "asset"}.${(asset.data.mimeType.split("/")[1] || "png").replace("mpeg", "mp3")}`);
    };
    ...
    // 收藏提示词单独一个视图。提前 return 而不是把现有内容整块包进条件里——
    // 后者要给几百行改缩进，纯风险没收益。
    if (view === "favorites") {
        return (
            <div className="flex h-full flex-col overflow-hidden bg-background text-stone-900 dark:text-stone-100">
                <main className="bg-paper-grid min-h-0 flex-1 overflow-y-auto bg-background px-6 py-8">
                    <div className="pb-8">
                        <div className="anim-rise mx-auto max-w-5xl text-center">
                            <h1 className="font-heading text-4xl font-medium tracking-wide text-stone-950 dark:text-stone-100">收藏提示词</h1>
                            <p className="mt-3 text-sm text-stone-500 dark:text-stone-400">在画布上觉得某次生成效果好，点节点的「收藏」，提示词和参考素材就会存到这里。</p>
                        </div>
                        <div className="anim-rise mx-auto mt-6 flex w-full max-w-5xl justify-center">
                            <Segmented value={view} onChange={(value) => setView(value as "assets" | "favorites")} options={viewOptions} />
                        </div>
                        <div className="anim-rise mx-auto mt-6 w-full max-w-5xl">
                            <PromptFavoritesTab columns={3} pageSize={12} />
                        </div>
                    </div>
                </main>
            </div>
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden bg-background text-stone-900 dark:text-stone-100">
            <main className="bg-paper-grid min-h-0 flex-1 overflow-y-auto bg-background px-6 py-8">
                <div className="pb-8">
                    <div className="anim-rise mx-auto max-w-5xl text-center">
                        <h1 className="font-heading text-4xl font-medium tracking-wide text-stone-950 dark:text-stone-100">我的素材</h1>
                    ...
                    <div className="anim-rise mx-auto mt-8 w-full max-w-2xl" style={{ "--rise-delay": "60ms" } as CSSProperties}>
                        <Input.Search
                            className="w-full [&_.ant-input-affix-wrapper]:!rounded-l-full [&_.ant-input-affix-wrapper]:!bg-card [&_.ant-input-affix-wrapper]:!pl-5 [&_.ant-input-affix-wrapper]:shadow-sm [&_.ant-input-search-button]:!rounded-r-full"
                            size="large"
                            allowClear
                    ...
                    <div className="anim-rise mx-auto mt-6 grid max-w-6xl gap-3 text-left" style={{ "--rise-delay": "120ms" } as CSSProperties}>
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div className="flex flex-wrap gap-4">
                                <button
                                    type="button"
                                    className="cursor-pointer text-sm font-medium text-stone-700 underline-offset-4 transition-colors hover:text-brand hover:underline focus-visible:text-brand focus-visible:underline focus-visible:outline-none dark:text-stone-300 dark:hover:text-brand"
                ...
                <div className="mx-auto flex max-w-7xl flex-col gap-5">
                    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        {visibleAssets.map((asset, index) => (
                            <AssetCard key={asset.id} asset={asset} riseDelay={Math.min(index, 12) * 40} onOpen={() => setPreviewAsset(asset)} onEdit={() => openEdit(asset)} onCopy={copyAssetText} onDownload={downloadImage} onDelete={() => setDeletingAsset(asset)} />
                        ))}
                    </div>
                    ...
                    <Form form={form} layout="vertical" requiredMark={false} initialValues={{ kind: "text", tags: [] }}>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <Form.Item name="source" label="来源">
                                <Input placeholder="手动添加 / 画布 / 提示词模板" />
                            </Form.Item>
...
function AssetCard({ asset, riseDelay = 0, onOpen, onEdit, onCopy, onDownload, onDelete }: { asset: Asset; riseDelay?: number; onOpen: () => void; onEdit: () => void; onCopy: (asset: Asset) => void; onDownload: (asset: Asset) => void; onDelete: () => void }) {
    const cover = asset.coverUrl || (asset.kind === "image" ? asset.data.dataUrl : "");
    const summary = assetSummary(asset);
    return (
        <Card
            className="anim-rise hover-lift group overflow-hidden !rounded-2xl !border-border bg-card"
            style={{ "--rise-delay": `${riseDelay}ms` } as CSSProperties}
        ...
        >
            <button type="button" className="block w-full text-left" onClick={onOpen}>
                <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                        ...
                        <Tag className="m-0 shrink-0 text-[11px]">{asset.kind === "image" ? "图片" : asset.kind === "video" ? "视频" : asset.kind === "audio" ? "音频" : asset.kind === "group" ? "组" : "文本"}</Tag>
                    </div>
            ...
            <div className="flex items-center gap-2 px-4 pb-4">
                ...
                {asset.kind !== "video" && asset.kind !== "audio" && asset.kind !== "group" ? (
            ...
            {asset ? (
                <div className="space-y-5">
                    ...
                    <div>
                        ...
                        <Space size={[4, 4]} wrap>
                            <Tag>{asset.kind === "image" ? "图片" : asset.kind === "video" ? "视频" : asset.kind === "audio" ? "音频" : asset.kind === "group" ? "组" : "文本"}</Tag>
                            {(asset.tags || []).map((tag) => (
                                <Tag key={tag}>{tag}</Tag>
                            ))}
                        </Space>
                    ...
                    <div className="rounded-lg border border-stone-200 p-4 dark:border-stone-800">
                        ...
                        {asset.kind === "text" ? (
                            ...
                            <Typography.Text className="mt-2 block">整组素材 · {(asset.data.nodes as unknown[])?.length || 0} 个节点（在画布右侧「素材」侧栏取用可整组调回）</Typography.Text>
                        ) : (
...
function assetSummary(asset: Asset) {
    ...
    if (asset.kind === "audio") {
        let dur = "";
        if (asset.data.durationMs) dur = `${Math.round(asset.data.durationMs / 1000)}s · `;
        return `${dur}${formatBytes(asset.data.bytes)} · ${asset.data.mimeType}`;
    }
    if (asset.kind === "group") return `整组素材 · ${(asset.data.nodes as unknown[])?.length || 0} 个节点`;
