...
export const CanvasResourceMentionTextarea = forwardRef<HTMLTextAreaElement, Props>(function CanvasResourceMentionTextarea({ value, references, onChange, onSubmit, onKeyDown, className, containerClassName, style, highlightLabels = true, ...props }, forwardedRef) {
    ...
    const syncMention = (nextValue: string, cursor: number) => {
        ...
        // @ 在任意位置都能触发（包括紧跟在文字后面），不再要求 @ 前是空格或行首。
    ...
    // 画布容器在原生阶段对 wheel 做了 preventDefault（阻止页面滚动），React 层的 onWheel stopPropagation
    // 来不及生效。在 textarea 自身原生阶段拦截，内容溢出时交还浏览器默认滚动。
    useEffect(() => {
        const textarea = textareaRef.current;
        if (!textarea) return;
        const handleWheel = (event: WheelEvent) => {
            if (textarea.scrollHeight > textarea.clientHeight) event.stopPropagation();
        };
    ...
    // 两件事保证叠加层与 textarea 逐行对齐（每次渲染后执行）：
    // 1. 排版属性直接拷贝 textarea 的计算样式——全局样式（如 antd reset 的 textarea line-height）
    //    可能只命中 textarea 不命中 div，行高差 1~2px 会随行数累积成明显的竖向光标错位。
    // 2. 叠加层重挂（选中文字后取消、引用激活变化）时 scrollTop 归零，需要和 textarea 再同步。
    useEffect(() => {
        const textarea = textareaRef.current;
    ...
    const mergedStyle = {
        ...
        // 高亮叠加时让 textarea 浮在叠加层之上，避免叠加层遮住光标（caret）。
        ...(showOverlay ? { background: "transparent", backgroundColor: "transparent", position: "relative", zIndex: 1 } : {}),
    } as CSSProperties;
    ...
    return (
        <div className={`relative h-full w-full ${containerClassName || ""}`}>
            {showOverlay ? (
                <div ref={overlayRef} className={`${className || ""} pointer-events-none absolute inset-0 overflow-hidden whitespace-pre-wrap break-words`} style={{ ...style, background: "transparent", borderColor: "transparent", color: theme.node.text }}>
                    <MentionHighlightText value={value || props.placeholder?.toString() || ""} labels={activeLabels} placeholder={!value} />
                    {/* textarea 对末尾换行会渲染出一行，pre-wrap 的 div 不会——补一个零宽字符使两者 scrollHeight 一致，否则滚到底时整体差一行。 */}
                    {value.endsWith("\n") ? "\u200B" : null}
                </div>
            ) : null}
            <textarea
                ...
                onKeyDown={(event) => {
                    ...
                    // Enter 用于换行（textarea 默认行为），仅点击生成按钮才提交；@ 菜单打开时的 Enter 选候选已在上面处理。
                    onKeyDown?.(event);
                }}
                onScroll={(event) => {
                    syncOverlayScroll();
        ...
        <>
            {value.split(pattern).map((part, index) =>
                labels.includes(part) ? (
                    // 高亮只用背景（不加 ring/padding/字重）：ring 是向外凸 1px 的 box-shadow，
                    // 在 Windows 较高字形 + 紧凑行高下会让相邻行高亮块竖直重叠，故去掉；
                    // 仅背景不占布局，保证叠加层与 textarea 逐字符对齐、不致光标错位。
                    <span key={`${part}-${index}`} className="rounded-[3px] bg-[#2563EB]/20 text-[#2563EB] dark:bg-[#3B82F6]/20 dark:text-[#3B82F6]">
                        {part}
                    </span>
                ) : (
...
function MentionMenu({ textarea, references, activeIndex, theme, onSelect }: { textarea: HTMLTextAreaElement; references: CanvasResourceReference[]; activeIndex: number; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onSelect: (reference: CanvasResourceReference) => void }) {
    ...
    return createPortal(
        <div
            ...
            className="anim-pop fixed z-[210] max-h-56 w-64 overflow-y-auto rounded-xl border border-border bg-popover p-1 shadow-[0_2px_8px_rgba(15,23,42,.08),0_18px_48px_rgba(15,23,42,.16)] backdrop-blur-md dark:shadow-[0_2px_8px_rgba(0,0,0,.3),0_18px_48px_rgba(0,0,0,.45)]"
            style={{ left, top, color: theme.node.text }}
