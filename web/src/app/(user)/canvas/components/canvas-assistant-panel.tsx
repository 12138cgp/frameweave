...
import { ArrowUp, Bot, History, ImageIcon, LoaderCircle, MessageSquare, PanelRightClose, Plus, RotateCcw, Settings2, Sparkles, Trash2, X } from "@/components/icons";
...
import { CreditSymbol, requestCreditCost, type ModelCreditCost } from "@/constant/credits";
...
import type { AgentCanvasApi } from "../agent/agent-ops";
import { CanvasAgentView } from "../agent/canvas-agent-view";
...
type CanvasAssistantPanelProps = {
    ...
    /** 画布助手用的画布 API。没传就不显示「助手」这个模式。 */
    agentApi?: AgentCanvasApi;
    onCollapse: () => void;
};

export function CanvasAssistantPanel({ nodes, selectedNodeIds, sessions, activeSessionId, onSelectNodeIds, onSessionsChange, onInsertImage, onInsertText, onPasteImage, onCollapseStart, onCollapse, agentApi }: CanvasAssistantPanelProps) {
    ...
    const [view, setView] = useState<"chat" | "history" | "agent">("chat");
            ...
            >
                ...
                <div className="flex items-center justify-between border-b px-4 py-3" style={{ borderColor: theme.node.stroke }}>
                    <div className="flex items-center gap-2 text-sm font-medium">
                        ...
                        {view === "history" ? "历史记录" : view === "agent" ? "排版助手" : "画布助手"}
                    </div>
                    <div className="flex items-center gap-1">
                        ...
                        {agentApi ? (
                            <Tooltip title={view === "agent" ? "返回对话" : "画布助手（帮你排版、连线）"}>
                                <Button
                                    type="text"
                                    shape="circle"
                                    className="!h-8 !w-8 !min-w-8"
                                    style={view === "agent" ? { ...iconButtonStyle, background: theme.node.activeStroke, color: theme.node.panel } : iconButtonStyle}
                ...
                {view === "agent" && agentApi ? (
                    <div className="min-h-0 flex-1">
                        <CanvasAgentView api={agentApi} selectedRefs={allSelectedReferences} />
                    </div>
                ) : null}

                <div className={view === "agent" ? "hidden" : "thin-scrollbar min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4"}>
                    {view === "history" ? (
                        <AssistantHistory
                        ...
                        <div className="anim-fade flex h-full flex-col items-center justify-center px-1 text-center">
                            <div className="font-heading relative text-4xl font-medium italic tracking-wide" style={{ color: theme.node.text }}>
                                <span>{APP_NAME}</span>
                                <DiaTextReveal className="absolute inset-0" colors={["#2563EB", "#14B8A6", "#6366F1"]} textColor="transparent" duration={1.8} startOnView={false} text={APP_NAME} />
                            </div>
                            <div className="font-heading mt-3 text-base italic tracking-wide opacity-60">One canvas, infinite ideas</div>
                        </div>
                    )}
                </div>
                ...
                {view === "chat" ? (
                    <AssistantComposer
                        ...
                        onOpenAgent={agentApi ? () => setView("agent") : undefined}
                    />
                ) : null}
...
function AssistantComposer({
    ...
    onPasteImage: (file: File) => void;
    // 用共享的 ModelCreditCost 而不是就地写个窄类型：窄类型会把 qualityRates 结构性地擦掉，
    // 于是这里显示的还是一口价，而后端已经按画质档扣——显示价与实扣价一旦分叉，就是直接的资损。
    modelCosts?: ModelCreditCost[];
}) {
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    const activeModel = mode === "image" ? config.imageModel || config.model : config.textModel || config.model;
    const credits = requestCreditCost({ channelMode: config.channelMode, modelCosts, model: activeModel, count: mode === "image" ? config.count : 1, quality: mode === "image" ? config.quality : undefined });
    ...
    return (
        <div className="px-2 pb-2" onWheelCapture={(event) => event.stopPropagation()}>
            ...
            <div className="rounded-[28px] border px-3 pb-3 pt-3 shadow-[0_14px_40px_rgba(15,23,42,.14)] dark:shadow-[0_14px_40px_rgba(0,0,0,.45)]" style={{ background: theme.toolbar.panel, borderColor: theme.node.stroke }}>
                <textarea
                    value={prompt}
                ...
                <div className="mt-2 flex items-center justify-between gap-2">
                    <div className="canvas-composer-tools flex min-w-0 flex-1 items-center gap-1">
                        ...
                        <AssistantModeSwitch mode={mode} theme={theme} onChange={onModeChange} onOpenAgent={onOpenAgent} />
                        {mode === "image" ? (
                            <>
                    ...
                    <Button
                        type="primary"
                        className="hover-lift !h-10 !min-w-16 shrink-0 !rounded-full !px-3"
...
function AssistantModeSwitch({ mode, theme, onChange, onOpenAgent }: { mode: AssistantMode; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onChange: (mode: AssistantMode) => void; onOpenAgent?: () => void }) {
    return (
        <div className="canvas-composer-mode-switch flex h-8 shrink-0 items-center rounded-full p-0.5" style={{ background: theme.node.fill }}>
            {[
                ...
                <Tooltip key={item.value} title={item.title}>
                    <button
                        type="button"
                        className="canvas-composer-mode-button flex h-7 cursor-pointer items-center justify-center gap-1 rounded-full border-0 bg-transparent transition-colors duration-200"
            ...
            {onOpenAgent ? (
                <Tooltip title="让助手帮你排版、连线（会先问过你再动手）">
                    <button
                        type="button"
                        className="canvas-composer-mode-button flex h-7 cursor-pointer items-center justify-center gap-1 rounded-full border-0 bg-transparent transition-colors duration-200"
        ...
        <div className="space-y-1">
            {sessions.map((session) => (
                <div key={session.id} className="group flex items-center gap-2 rounded-lg px-2 py-1.5 transition hover:bg-black/5 dark:hover:bg-white/10" style={session.id === activeSession?.id ? { background: theme.node.fill } : undefined}>
                    <input type="checkbox" className="size-4 accent-[#2563EB] dark:accent-[#3B82F6]" checked={checkedIds.includes(session.id)} onChange={(event) => onToggleChecked(session.id, event.target.checked)} />
