...
import { Brush, Eraser, RotateCcw, WandSparkles, X } from "@/components/icons";
...
// 遮罩涂抹色：主色淡涂（与品牌色一致）
const maskFillColor = "rgba(37, 99, 235, .40)";
    ...
    return (
        <Modal title={null} open={open && Boolean(dataUrl)} onCancel={onClose} footer={null} width={980} centered destroyOnHidden>
            <div className="grid gap-5 lg:grid-cols-[minmax(360px,1fr)_320px]">
                <div className="flex min-h-[360px] items-center justify-center rounded-xl border bg-transparent p-0">
                ...
                <div className="flex min-h-[360px] flex-col gap-5">
                    <div>
                        <h2 className="font-heading text-xl font-medium tracking-wide">局部遮罩编辑</h2>
                    ...
                    <div className="space-y-2">
                        ...
                        />
                        {error ? <div className="text-xs font-medium text-[#DC2626] dark:text-[#EF4444]">{error}</div> : null}
                    </div>
