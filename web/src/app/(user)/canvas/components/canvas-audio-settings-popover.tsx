...
import { audioFormatLabel, audioSampleRateLabel, audioSpeedLabel, audioVoiceLabel, isSeedAudioModel, normalizeAudioSampleRate } from "@/lib/audio-generation";
import { canvasThemes } from "@/lib/canvas-theme";
import { useThemeStore } from "@/stores/use-theme-store";
import type { AiConfig } from "@/stores/use-config-store";

export type CanvasAudioSettingKey = "audioVoice" | "audioFormat" | "audioSpeed" | "audioInstructions" | "audioSampleRate" | "audioLoudness" | "audioPitch";
        ...
        <>
            <span ref={buttonRef} className="inline-flex min-w-0">
                <Button size="small" type="text" className={`${buttonClassName || "!h-8 !max-w-[170px] !justify-start !rounded-full !px-2.5"} !transition hover:!brightness-[.96] dark:hover:!brightness-110`} style={{ background: theme.node.fill, color: theme.node.text }} icon={<Settings2 className="size-3.5" />} onClick={() => setOpen((current) => !current)}>
                    <span className="truncate">{audioSettingsSummary(config)}</span>
                </Button>
            </span>
            {panel}
        </>
...
// audioSettingsSummary 按钮上那行摘要。seed-audio 没有音色，显示的是「采样率 · 格式 · 语速」；
// 老的 TTS 维持原样「音色 · 格式 · 语速」。用 if/return 不写三元，规避 bun 构建期 SIGILL。
function audioSettingsSummary(config: AiConfig) {
    const model = config.model || config.audioModel;
    if (isSeedAudioModel(model)) {
        const rate = audioSampleRateLabel(normalizeAudioSampleRate(config.audioFormat, config.audioSampleRate));
        return `${rate} · ${audioFormatLabel(config.audioFormat)} · ${audioSpeedLabel(config.audioSpeed)}`;
    }
    return `${audioVoiceLabel(config.audioVoice, model)} · ${audioFormatLabel(config.audioFormat)} · ${audioSpeedLabel(config.audioSpeed)}`;
}
