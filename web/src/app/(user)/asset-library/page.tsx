...
import { Copy, FolderPlus, Search } from "@/components/icons";
            ...
            <main className="bg-paper-grid min-h-0 flex-1 overflow-y-auto bg-background px-6 py-8">
                <div className="pb-8">
                    <div className="anim-rise mx-auto max-w-5xl text-center">
                        <h1 className="font-heading text-4xl font-medium tracking-wide text-stone-950 dark:text-stone-100">素材库</h1>
                    ...
                    <div className="anim-rise mx-auto mt-8 w-full max-w-2xl" style={{ "--rise-delay": "60ms" } as CSSProperties}>
                        <Input
                            size="large"
                            className="w-full !rounded-full !bg-card !px-5 shadow-sm"
                            prefix={<Search className="mr-1 size-4 text-stone-400" />}
                            value={keyword}
                            placeholder="按标题查询"
                            onChange={(event) => {
                                setPage(1);
                    ...
                    <div className="anim-rise mx-auto mt-6 max-w-6xl space-y-3" style={{ "--rise-delay": "120ms" } as CSSProperties}>
                ...
                <div className="mx-auto flex max-w-7xl flex-col gap-5">
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-5">
                        {items.map((asset, index) => (
                            <LibraryCard key={asset.id} asset={asset} riseDelay={Math.min(index, 12) * 40} onOpen={() => setSelectedAsset(asset)} onAdd={() => void saveToMyAssets(asset)} />
                        ))}
                    </div>
...
function LibraryCard({ asset, riseDelay = 0, onOpen, onAdd }: { asset: AssetLibraryItem; riseDelay?: number; onOpen: () => void; onAdd: () => void }) {
