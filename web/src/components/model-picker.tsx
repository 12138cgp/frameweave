...
import { selectableModelsByCapability, useModelLabels, type AiConfig, type ModelCapability } from "@/stores/use-config-store";
...
export function ModelPicker({ config, value, onChange, capability, className, fullWidth = false, placeholder = "选择模型", onMissingConfig }: ModelPickerProps) {
    ...
    if (current) triggerText = labels[current] || current;
    ...
    return (
        ...
        >
            ...
            <SelectContent
                data-canvas-no-zoom
                className="z-[1200] w-80 max-w-[calc(100vw-24px)] rounded-xl border border-border bg-popover p-1 shadow-[0_2px_6px_rgba(15,23,42,0.08),0_16px_40px_rgba(15,23,42,0.18)] dark:shadow-[0_2px_6px_rgba(0,0,0,0.4),0_16px_40px_rgba(0,0,0,0.45)]"
                position="popper"
                align="start"
                side="bottom"
                sideOffset={6}
            ...
            >
                {options.length ? (
                    options.map((model) => (
                        <SelectItem key={model} value={model} textValue={`${labels[model] || model} ${model}`}>
                            <ModelLabel model={model} label={labels[model]} />
                        </SelectItem>
                    ))
                ) : (
