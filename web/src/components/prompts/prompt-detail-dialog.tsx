...
import { Copy, FolderPlus } from "@/components/icons";
...
export function PromptDetailDialog({ prompt, onClose, onCopy, onSaveAsset }: { prompt: Prompt | null; onClose: () => void; onCopy: (prompt: string) => void; onSaveAsset?: (prompt: Prompt) => void }) {
    return (
        <>
            <Modal title={<span className="font-heading text-base font-medium tracking-wide">{prompt?.title}</span>} open={Boolean(prompt)} onCancel={onClose} footer={null} width={860}>
                {prompt ? (
                    <>
                        <div className="anim-fade grid gap-5 md:grid-cols-[300px_minmax(0,1fr)]">
                            <div className="space-y-3">
                                <PromptCover src={prompt.coverUrl} alt={prompt.title} zoomable className="rounded-xl ring-1 ring-border" />
